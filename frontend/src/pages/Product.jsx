    ...
    <div className="max-w-7xl mx-auto px-4 md:px-6 py-8 md:py-12">
      <div className="grid md:grid-cols-2 gap-8 md:gap-14">
        ...
        >
          ...
          <h1
            className="font-heading text-3xl md:text-4xl font-extrabold mt-1 leading-tight"
            data-testid="product-name"
          >
            {p.name}
          </h1>
          <div className="flex items-center gap-3 mt-3 text-sm">
            <div className="flex items-center gap-1 px-2 py-0.5 rounded-md bg-primary/10 text-primary font-semibold">
          ...
          <div className="mt-6 grid grid-cols-3 gap-3">
            {[
              { i: Truck, t: `${p.delivery_min}m delivery` },
              { i: Shield, t: "100% Fresh" },
              { i: Award, t: "Quality Assured" },
            ].map((x, i) => (
          ...
          <div className="mt-8 flex items-center gap-3">
            ...
            <motion.button
              ...
              className="flex-1 flex items-center justify-center gap-2 py-3.5 rounded-full bg-primary text-primary-foreground font-heading font-semibold hover:opacity-90"
            ...
            <motion.button
              whileTap={{ scale: 0.96 }}
              onClick={buyNow}
              className="flex-1 flex items-center justify-center gap-2 py-3.5 rounded-full bg-secondary text-secondary-foreground font-heading font-semibold hover:opacity-90"
            ...
            <button
              ...
              className="w-12 h-12 rounded-full border flex items-center justify-center hover:bg-muted"
              data-testid="wishlist-detail-btn"
            >
              <Heart
                className={`w-5 h-5 ${inWish ? "fill-secondary text-secondary" : ""}`}
              />
            </button>
          </div>
              ...
              <div>
                <div className="flex items-center text-secondary">
                  {[1, 2, 3, 4, 5].map((i) => (
                    <Star
                      key={i}
                      className={`w-4 h-4 ${i <= Math.round(p.rating) ? "fill-current" : ""}`}
                    />
                  ))}
                </div>
      ...
      {data.similar?.length > 0 && (
        <div className="mt-16">
          <h2 className="font-heading text-2xl md:text-3xl font-extrabold flex items-center gap-2">
            Similar Products <ChevronRight className="w-6 h-6" />
          </h2>
          <div className="mt-6 grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4 md:gap-6">
            {data.similar.map((sp, i) => (
              <ProductCard key={sp.id} p={sp} index={i} />
            ))}
          </div>
        </div>
      )}
